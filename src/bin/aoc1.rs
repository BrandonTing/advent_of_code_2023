...
fn parse_line(line: &str) -> Result<u32, Error> {
    ...
    for letter in DigitLetters::iter() {
        if new_line.contains::<&str>(letter.to_string().as_str()) {
            // "sevenine" is 79.
            let mut to_str = letter.to_string().chars().nth(0).unwrap().to_string();
            to_str.push_str(letter.clone().into_digit().as_str());
            to_str.push_str(
                letter
                    .clone()
                    .to_string()
            ...
            new_line = new_line.replace(letter.to_string().as_str(), to_str.as_str());
        };
    }
    println!("{:?}", new_line);

    let mut int_str: String = new_line
        .chars()
