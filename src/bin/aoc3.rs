...
fn check_adjacent_to_symbol(
    ...
    if has_down {
        let down_char = lines
            ...
            .nth(char_index)
            .unwrap();
        println!("down_char {:?}", down_char);
        println!(
            "check_char_is_symbol {:?}",
            check_char_is_symbol(&down_char)
        );
...
fn parse_line(lines: &Vec<&str>, line: &str, line_index: usize) -> Vec<u32> {
    let mut nums: Vec<u32> = vec![];
    let mut tmp = String::from("");
    let mut valid = false;
    let chars: Vec<char> = line.chars().collect();
    chars.iter().enumerate().for_each(|(char_index, char)| {
        // println!("char: {:?}, valid: {:?}", char, valid);
        // check if adjacent to symbol
        if char.to_digit(10).is_some() {
            println!("char {:?}", char);
            if !valid {
                valid = check_adjacent_to_symbol(&chars, char_index, lines, line_index);
            }
            tmp.push(*char);
        } else {
            println!("tmp {:?}", tmp);
            if valid {
                if tmp != "" {
                    nums.push(tmp.parse().unwrap_or(0));
                }
                valid = false;
            }
            tmp = String::from("");
...
fn main() {
    ...
    let counts = lines
        .iter()
        .enumerate()
        .flat_map(|(i, line)| parse_line(&lines, line, i))
        .fold(0, |sum, count| {
            println!("count  {:?}", count);
            return sum + count;
        });
