...
fn check_char_is_symbol(char: &char) -> bool {
    char.to_digit(10).is_none() && char.to_string() != ".".to_string()
}

fn check_adjacent_to_symbol(
    chars: &Vec<char>,
    char_index: usize,
    lines: &Vec<&str>,
    line_index: usize,
) -> (bool, Vec<GearPosition>) {
    println!("index {:?}", char_index);
    let has_left = char_index > 0;
    let has_right = char_index < chars.len() - 1;
    let has_up = line_index > 0;
    let has_down = line_index < lines.len() - 1;
    println!("up {:?}", has_up);
    println!("down {:?}", has_down);
    let mut is_adjacent: bool = false;
    let gear = "*";
    let mut gear_positions: Vec<GearPosition> = vec![];
    // check before and after
    if has_left {
        let left_char = chars.get(char_index - 1).unwrap();
        if check_char_is_symbol(&left_char) {
            is_adjacent = true;
            if left_char.to_string() == gear.to_string() {
    ...
    if has_right {
        let right_char = chars.get(char_index + 1).unwrap();
        if check_char_is_symbol(&right_char) {
            is_adjacent = true;
            if right_char.to_string() == gear.to_string() {
                gear_positions.push(GearPosition {
                    x: char_index + 1,
                    y: line_index,
                });
    ...
    if has_up {
        let up_char = lines
            ...
            .nth(char_index)
            .unwrap();
        if check_char_is_symbol(&up_char) {
            is_adjacent = true;
            if up_char.to_string() == gear.to_string() {
                gear_positions.push(GearPosition {
                    x: char_index,
                    y: line_index - 1,
                });
    ...
    if has_down {
        ...
        if check_char_is_symbol(&down_char) {
            is_adjacent = true;
            if down_char.to_string() == gear.to_string() {
                gear_positions.push(GearPosition {
                    x: char_index,
                    y: line_index + 1,
                });
    ...
    // check diagonally
    if has_left && has_up {
        let up_left_char = lines
            .get(line_index - 1)
            .unwrap()
            .chars()
            .nth(char_index - 1)
            .unwrap();
        if check_char_is_symbol(&up_left_char) {
            is_adjacent = true;
            if up_left_char.to_string() == gear.to_string() {
                gear_positions.push(GearPosition {
                    x: char_index - 1,
                    y: line_index - 1,
                });
    ...
    if has_left && has_down {
        let down_left_char = lines
            .get(line_index + 1)
            .unwrap()
            .chars()
            .nth(char_index - 1)
            .unwrap();
        if check_char_is_symbol(&down_left_char) {
            is_adjacent = true;
            if down_left_char.to_string() == gear.to_string() {
                gear_positions.push(GearPosition {
                    x: char_index - 1,
                    y: line_index + 1,
                });
    ...
    if has_right && has_up {
        let up_right_char = lines
            .get(line_index - 1)
            .unwrap()
            .chars()
            .nth(char_index + 1)
            .unwrap();
        if check_char_is_symbol(&up_right_char) {
            is_adjacent = true;
            if up_right_char.to_string() == gear.to_string() {
                gear_positions.push(GearPosition {
                    x: char_index + 1,
                    y: line_index - 1,
                });
    ...
    if has_right && has_down {
        let down_right_char = lines
            .get(line_index + 1)
            .unwrap()
            .chars()
            .nth(char_index + 1)
            .unwrap();
        if check_char_is_symbol(&down_right_char) {
            is_adjacent = true;
            if down_right_char.to_string() == gear.to_string() {
                gear_positions.push(GearPosition {
                    x: char_index + 1,
                    y: line_index + 1,
                });
...
fn parse_line(gears: &mut Vec<Gear>, lines: &Vec<&str>, line: &str, line_index: usize) -> Vec<u32> {
    let mut nums: Vec<u32> = vec![];
    let mut tmp = String::from("");
    let mut valid = false;
    let chars: Vec<char> = line.chars().collect();
    let mut gear_positions: Vec<GearPosition> = vec![];
    chars.iter().enumerate().for_each(|(char_index, char)| {
        // check if adjacent to symbol
        if char.to_digit(10).is_some() {
            if !valid {
                let check = check_adjacent_to_symbol(&chars, char_index, lines, line_index);
                valid = check.0;
                gear_positions = check.1;
            }
            tmp.push(*char);
        } else {
            if valid {
                if tmp != "" {
                    let num = tmp.parse().unwrap_or(0);
                    nums.push(num);
                    // update gears
                    for gear_position in &gear_positions {
                        match gears.iter_mut().find(|gear| {
                            gear.position.x == gear_position.x && gear.position.y == gear_position.y
                        }) {
                            Some(v) => v.part_numbers.push(num),
                            None => gears.push(Gear {
                                position: GearPosition {
                                    x: gear_position.x,
                                    y: gear_position.y,
                                },
                                part_numbers: vec![num],
                            }),
                        }
                    }
                }
...
fn main() {
    let lines = get_inputs().lines().collect::<Vec<_>>();
    let mut gears: Vec<Gear> = vec![];
    let counts = lines
        .iter()
        .enumerate()
        .flat_map(|(i, line)| parse_line(&mut gears, &lines, line, i))
        .fold(0, |sum, count: u32| sum + count);
    println!("counts {:?}", counts);
    println!("gears {:?}", gears);
    let gear_sum = gears
        .iter()
        .filter(|x| x.part_numbers.len() == 2)
        .map(|gear| gear.part_numbers.get(0).unwrap() * gear.part_numbers.get(1).unwrap())
        .fold(0, |sum, count: u32| sum + count);
